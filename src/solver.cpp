...
namespace snek {
    ...
    std::pair<int, int> Solver::get_next_move() {
        ...
        bool can_do_shortcuts = (orig_dist_food < orig_dist_tail) &&
                                (m_get_board_to_snake_ratio() <
                                 game_config::FOLLOW_EXACT_THRESHOLD);
            ...
            if (((0 <= ynew && ynew < this->m_board->height()) &&
                ...
                if (can_do_shortcuts ||
                    ((this->m_path.get_path()[ynew][xnew] ==
                      this->m_path.get_path()[pos.first][pos.second] + 1) ||
    ...
    std::size_t Solver::m_get_dist_to_point(
    ...
    std::size_t Solver::m_get_dist_to_food(
        std::pair<std::size_t, std::size_t> start_pos) {
        return this->m_get_dist_to_point(start_pos,
                                         this->m_board->get_food_location());
    }

    std::size_t Solver::m_get_dist_to_tail(
        std::pair<std::size_t, std::size_t> start_pos) {
        return this->m_get_dist_to_point(start_pos,
                                         this->m_board->get_snake().back());
    }
